"""Help documentation management for Claude Headspace.
...
# Path to help documentation directory
HELP_DOCS_DIR = Path(__file__).parent.parent / "docs" / "application"
...
def load_help_page(slug: str) -> Optional[dict]:
...
def search_help(query: str) -> list[dict]:
    ...
    for md_file in HELP_DOCS_DIR.glob("*.md"):
        ...
        if score > 0:
            ...
            snippet = extract_snippet(markdown_content, query, query_words)
...
def extract_snippet(content: str, query: str, query_words: list[str], max_length: int = 150) -> str:
...
def get_help_page_content(slug: str) -> Optional[str]:
